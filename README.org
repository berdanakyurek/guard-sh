...
Commands in your *whitelist* skip the LLM entirely. Commands in your *blacklist* are always flagged without an LLM call.
...
- Go 1.22+
- bash or zsh
- API key(s) for one or more of the LLMs listed under the [[Supported LLMs]] section.
...
Then edit your config and set your API key:
...
providers:
  gemini:
    ...
    model: gemini-2.0-flash-lite   # optional, this is the default
...
# Commands always flagged — LLM is never called, confirmation always required.
# If ANY part of a chained command matches, the whole command is flagged.
# command_blacklist:
#   - rm
#   - mkfs
#+end_src
...
Commands in the blacklist are always flagged. The LLM is never called.
...
- ~rm file.txt~ → flagged immediately: ~'rm' is blacklisted. Are you sure? [Y/n]~
- ~ls && rm -rf /~ → ~rm~ is blacklisted, so the whole command is flagged
- For chained commands, *any* part matching is enough to flag the command
