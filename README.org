...
- Providers can be added interactively with =guard-sh provider add= (see *Providers* below).
- The whitelist can also be managed with =guard-sh whitelist= (see *Whitelist* below).
- Cache settings can also be toggled with =guard-sh cache= (see *Cache* below).
