...
This will create the config dir, write the shell integration scripts, and add them to your =.bashrc= / =.zshrc=.
...
guard-sh can redact sensitive values from commands before sending them to any LLM provider. Patterns are Go regular expressions stored in =redact_patterns= in your config. Matches are replaced with =[REDACTED]=.
...
You can add your own patterns. For example:
...
Redaction is enabled per provider. The default is on for all providers. Toggle it interactively:

#+begin_src bash
guard-sh redact pattern on    # interactively enable pattern redaction for a provider
guard-sh redact pattern off   # interactively disable pattern redaction for a provider
#+end_src
...
providers:
  gemini:
    api_key: YOUR_KEY
    pattern_based_redaction_enabled: true   # default, can omit
  ollama:
    host: http://localhost:11434/api/chat
    pattern_based_redaction_enabled: false  # disable for local model
#+end_src
...
#+begin_example
  ...
  1  gemini
     ...
     pattern based redaction  ● on

  2  ollama
     model                llama3.2
     pattern based redaction  ○ off
...
Creates =~/.config/guard-sh/= with the default config and prompt, writes shell integration scripts, and adds them to your =.bashrc= / =.zshrc=. Existing files are not overwritten. This is the only step needed after downloading a pre-built binary.
